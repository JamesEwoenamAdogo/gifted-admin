...
const BILLS = [
    {invoiceNo : "#4567", amount : "250", description : "Olympiad", status : "Pending", generatedOn : moment(new Date()).add(-30*1, 'days').format("DD MMM YYYY"),  paidOn : "-"},
...
function Billing(){
    ...
    useEffect(()=>{
        const fetchTransactions = async()=>{
            const allTransactions = await axios.get("/all-transactions")
            console.log(allTransactions.data.transactions)
            setBills(()=>{return [...allTransactions.data.transactions]})
        }
                    ...
                    <tbody>
                        {
                            bills.map((l, k) => {
                                return(
                                    <tr key={k}>
                                    <td>{l._id}</td>
                                    <td>{`${new Date(l.createdAt).toString().split("+")[0]}`}</td>
                                    <td>{l.description}</td>
                                    <td>{l.amount}</td>
